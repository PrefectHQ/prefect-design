...
export function useAttrsStylesClassesAndListeners(): UseAttrsStylesAndListeners {
  ...
  watchEffect(() => {
    for (const attr in attrs.value) {
      if (isListener(attr)) {
        listeners.value[attr] = attrs.value as unknown as Record<string, unknown>[typeof attr]
      } else {
        nonListenerAttrs.value[attr] = attrs.value as unknown as Record<string, unknown>[typeof attr]
      }
    }
  })
